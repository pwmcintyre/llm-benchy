...
PY
    ...
    startup=$((ended - started))
    status="failed"
    docker logs --tail 220 qwen-vllm > "benchmark/results/${slug}_failure.log" 2>&1 || true
  ...
  printf "%s|%s|%s|%s|%s|%s|%s\n" "$slug" "$model" "$status" "$startup" "$max_len" "$max_seqs" "$note" >> /tmp/qwen35_matrix_rows.txt
}

: > /tmp/qwen35_matrix_rows.txt

# Official Qwen3.5 sizes up to 9B
...
# Optional quantized 9B fallback for 12GB cards
...

{
  ...
  echo "|---|---|---:|---:|---:|---:|---|"
  cat /tmp/qwen35_matrix_rows.txt
